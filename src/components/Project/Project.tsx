...
export default function Project({ projects: initialProjects }: TProjectProps) {
  ...
  const handleEditProject = (projectId: string) => {
    ...
    if (projectToEdit) {
      setNewProject({
        ...
        whatILearned: projectToEdit.whatILearned || [""],
        futureImprovements: projectToEdit.futureImprovements || [""],
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      if (editingProjectId) {
          ...
          prev.map((project) =>
            project.id === editingProjectId
              ? {
                  ...project,
                  ...cleanedProject,
                  id: project.id, // ensure id is always present and not undefined
                  title: cleanedProject.title ?? project.title,
                  overview: cleanedProject.overview ?? project.overview,
                  description:
                    cleanedProject.description ?? project.description,
                  projectImage:
                    cleanedProject.projectImage ?? project.projectImage,
                  techStack: cleanedProject.techStack ?? project.techStack,
                  features: cleanedProject.features ?? project.features,
                  whatILearned:
                    cleanedProject.whatILearned ?? project.whatILearned,
                  futureImprovements:
                    cleanedProject.futureImprovements ??
                    project.futureImprovements,
                  liveURL: cleanedProject.liveURL ?? project.liveURL,
                  gitHubURL: cleanedProject.gitHubURL ?? project.gitHubURL,
                  authorId: cleanedProject.authorId ?? project.authorId,
                  date_time: cleanedProject.date_time ?? project.date_time,
                }
              : project
          )
        );
        ...
        setProjects((prev) => [
          {
            id: cleanedProject.id ?? `project-${Date.now()}`,
            title: cleanedProject.title ?? "",
            overview: cleanedProject.overview ?? "",
            description: cleanedProject.description ?? "",
            projectImage:
              cleanedProject.projectImage ??
              "/placeholder.svg?height=400&width=600",
            techStack: cleanedProject.techStack ?? [],
            features: cleanedProject.features ?? [],
            whatILearned: cleanedProject.whatILearned ?? [],
            futureImprovements: cleanedProject.futureImprovements ?? [],
            liveURL: cleanedProject.liveURL ?? "",
            gitHubURL: cleanedProject.gitHubURL ?? "",
            authorId: cleanedProject.authorId ?? "uy",
            date_time: cleanedProject.date_time ?? "",
            is_public: true,
            heroSection: false,
            isDeleted: false,
            status: "active",
  ...
  const handleDeleteProject = async (projectId: string) => {
    ...
    try {
      ...
      setProjects((prev) =>
        prev.filter((project: TProject) => project.id !== projectId)
      );
      ...
      {(isAddingProject || editingProjectId) && (
        <div className="bg-[#120b20] border border-[#2d1b4d] rounded-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              <label htmlFor="techStack" className="block text-gray-400 mb-2">
                techStack
              </label>
              <select
                id="techStack"
          ...
          <div className="mb-6">
            ...
            {newProject.futureImprovements?.map((feature, index) => (
              <div key={index} className="flex items-center gap-2 mb-2">
                <input
                  type="text"
                  value={feature}
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-red-500 p-1"
                  disabled={
                    newProject.features && newProject.features.length <= 1
                  }
                >
                  <Trash2 size={16} />
                </button>
          ...
          <div className="mb-6">
            ...
            {newProject.whatILearned?.map((learning, index) => (
              <div key={index} className="flex items-center gap-2 mb-2">
                <input
                  type="text"
                  value={learning}
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-red-500 p-1"
                  disabled={
                    newProject.whatILearned &&
                    newProject.whatILearned.length <= 1
                  }
                >
                  <Trash2 size={16} />
                </button>
          ...
          <div className="mb-6">
            ...
            {newProject.futureImprovements?.map((futureImprovements, index) => (
              <div key={index} className="flex items-center gap-2 mb-2">
                <input
                  type="text"
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-red-500 p-1"
                  disabled={
                    newProject.futureImprovements &&
                    newProject.futureImprovements.length <= 1
                  }
                >
                  <Trash2 size={16} />
                </button>
